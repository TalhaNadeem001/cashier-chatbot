...
from src.chatbot.api.schema import AddItemsResult, Message, ModifyItem, OrderItem, SwapItems
from src.chatbot.application.openai_messages import openai_chat_history_from_messages
...
async def extract_modify_items(
    ...
    system = (
        EXTRACT_MODIFY_ITEMS_SYSTEM_PROMPT
        .replace("{order_state}", str(order_state))
        .replace("{menu_context}", menu_repository.get_menu_context())
    )
    messages = [
        {"role": "system", "content": system},
        *history,
